...
function populateTapForm(tapData) {
    tapData.forEach((tap, index) => {
        if (index < 5) { // Only populate first 5 rows
            ...
            const ibuInput = document.querySelector(`input[name="taplist[${index}][ibu]"]`);
            const ebcInput = document.querySelector(`input[name="taplist[${index}][ebc]"]`);
            const price05lInput = document.querySelector(`input[name="taplist[${index}][price_05l]"]`);
            const price03lInput = document.querySelector(`input[name="taplist[${index}][price_03l]"]`);
            ...
            if (ibuInput) ibuInput.value = tap.ibu || '';
            if (ebcInput) ebcInput.value = tap.ebc || '';
            if (price05lInput) price05lInput.value = tap.price_05l || '';
            if (price03lInput) price03lInput.value = tap.price_03l || '';
        }
    });
}
